...
function buildAgentResponse(
  ...
  const identityLine = `Selun CDP identity is online on ${network} with wallet ${shortenAddress(walletAddress)}.`;
  ...
  return [
    ...
    "Selun decisions are generated by backend doctrine with Coinbase CDP wallet execution.",
  ].join("\n");
}
...
export async function POST(req: Request): Promise<NextResponse<AgentResponse>> {
  ...
  try {
    ...
    const walletAddress = walletData.data?.walletAddress;
    const network = walletData.data?.network;
    if (!walletResponse.ok || !walletAddress || !network) {
      return NextResponse.json(
        {
          success: false,
          error: walletData.error ?? "Selun CDP wallet is unavailable.",
        },
        { status: walletResponse.status || 502 },
      );
    }

    const responseText = buildAgentResponse(payload, walletAddress, network);
