...
export async function POST(req: Request) {
  ...
  try {
    const response = await fetch(`${getBackendBaseUrl()}/agent/admin/withdraw-treasury`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Failed to submit treasury withdrawal.",
      },
      { status: 502 },
    );
  }
